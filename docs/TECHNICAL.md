...
`npm test`：81 個測試，包含兩個執行器的 contract、OAuth／HTTP／stdio、agent hub（用假的 agent 在真的 git repo 裡跑）、檔案傳輸，以及所有卡片的 script 語法檢查、專案授權的邊界（`C:\work` 不會放行 `C:\work-secrets`）、文件是否過期。
